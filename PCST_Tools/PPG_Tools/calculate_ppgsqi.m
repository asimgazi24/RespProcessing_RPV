function [ppgsqi_numeric, sqimatrix_all, annot_all]= ...
    calculate_ppgsqi(PPGann,Waveform,fs,windowTime)
...
% analysis PPG_SQI
windowlen=windowTime*fs; % window is windowTime s long
template=[];
beat_i=1;
...
for j=1:ceil(length(Waveform)/windowlen)
    try
        ...
        % PPG SQI analysis
        [annot, sqimatrix, template, valid] = ...
            PPG_SQI_buf(wave,anntime,template,windowTime*fs,fs);
...
sqimatrix_all = sqimatrix_all(:, 1:3);
ppgsqi_numeric = round(mean(sqimatrix_all,2));
end
