function [position,positionaux,messages] = wavedet_3D_detector(sig_all, ext_anot, heasig, messages )
...
if (size(sig_all,1) < size(sig_all,2))
   sig_all = sig_all'; 
end
...
if ~isfield(messages.setup.wavedet,'filter_bank_design') || messages.setup.wavedet.filter_bank_design~=1 % Rute 24/11/11
    ...
    [q1,q2,q3,q4,q5,messages] = qspfilt5_detector(messages.setup.wavedet.freq,messages);  % WT equivalent filters % Rute 22/05/02
    if messages.status==1
...
%     eps= 0.5*sqrt(nanmean(swt));
    eps= rowvec_detector(0.5*sqrt(nanmedian(swt))) .* rowvec_detector(messages.setup.wavedet.QRS_detection_thr);
    eps(4) = eps(4)*2;
...
%         position1.qrs(sel)=ext_anot(sel); %4ABRIL2011
        position1.qrs = rowvec_detector(ext_anot);
    ...
    if( ~messages.setup.wavedet.QRS_detection_only )
        ...
        if ~isempty(time) %2JUN08 RUTE
            ...
            if ~isempty(intervalo1)
                [position1,qrspiconset1,qrspicoffset1,messages]= qrswavef_detector(heasig,samp,time1,position1,w1,intervalo1,messages);
            ...
            if size(sig,2)>1
                [position2,qrspiconset2,qrspicoffset2,messages]= qrswavef_detector(heasig,samp,time2,position2,w2,intervalo2,messages);
                ...
                if size(sig,2)>2 && ~isempty(intervalo3)
                    [position3,qrspiconset3,qrspicoffset3,messages]= qrswavef_detector(heasig,samp,time3,position3,w3,intervalo3,messages); %#ok<ASGLU>
...
%                 end
            ...
            if qrs_flag~=2
                %%%%%%%%%%%%%%%%%%%%%%%%%%%% T wave
                if ~isempty(time1)
                    [position1,janelas1,messages]= twavef_detector(heasig,samp,time1,position1,w1,intervalo1,messages); %21AGO09
                end
                %end
                if size(sig,2)>1 && ~isempty(time2)
                    if length(time2(1,:))>1%23SET08
                        [position2,janelas2,messages]= twavef_detector(heasig,samp,time2,position2,w2,intervalo2,messages);%21AGO09
                    end
                    if size(sig,2)>2  && ~isempty(intervalo3)
                        if length(time3(1,:))>1%23SET08
                            [position3,janelas3,messages]= twavef_detector(heasig,samp,time3,position3,w3,intervalo3,messages);%21AGO09
                        end
                    end
                end
                if size(sig,2)>1
                ...
                [position1,picon_all,picoff_all,messages]= pwavef_detector(heasig,samp,time1,position1,w1,intervalo1,ultimo_anot,messages); %#ok<ASGLU>
                if size(sig,2)>1
                    [position2,picon_all,picoff_all,messages]= pwavef_detector(heasig,samp,time2,position2,w2,intervalo2,ultimo_anot,messages); %#ok<ASGLU>
                    if size(sig,2)>2 & ~isempty(intervalo3) %#ok<AND2>
                        [position3,picon_all,picoff_all,messages]= pwavef_detector(heasig,samp,time3,position3,w3,intervalo3,ultimo_anot,messages); %#ok<ASGLU>
