...
%% Add necessary toolboxes and code libraries to the MATLAB path
% Find path where executeGui script lies

dependencies = mfilename('fullpath');
dependencies = extractBefore(dependencies, [filesep, 'gui_process']);

% Add this folder (and all subfolders using genpath) to MATLAB path
...
[yr, mon, ~] = ymd(datetime);
HRVparams = InitializeHRVparams([num2str(mon), '_', num2str(yr)]);
...
% Now, because we may use a for loop, we have to implement a
