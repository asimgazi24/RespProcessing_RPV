function [expTime, flaggedIBIs] = extractTe(IBIs, onsetTimes, numDevs,varargin)
% This function takes as input inter-breath intervals along with associated
% onset times and returns expiration time by finding the length of time
% from beginning of the IBI to the onset of the next breath
...
% ---------- Outputs ----------- %
% expTime: Nx2 array, where first column stores IBI time and second column
...
% Initalize expiration times vector
expTime = zeros(size(IBIs, 1), 2);

% Store time
expTime(:, 1) = IBIs(:, 1);

% Compute expiration times
% To find expiratino time, we need to find the time difference between
% onset time of second breath and the time of the first breath. To find the
% time of first breath, we can just take time of second breath (IBI time)
% and subtract away the IBI value (length of time). We can then subtract
% this quantity away from the onset time
expTime(:, 2) = onsetTimes - (IBIs(:, 1) - IBIs(:, 2));
...
if flagOutlier
    ...
    [stg1_rmvd, rmvd_bool] = rmoutliers(expTime(:, 2), ...
