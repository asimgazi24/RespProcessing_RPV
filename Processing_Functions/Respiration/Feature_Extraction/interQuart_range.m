function [windowed_IQR, numWindRmvd] = ...
    interQuart_range(data_time_array, windows, mean_IBI)
...
% and last column contains the IQR computed for those windows
...
% ----------- Outputs ---------------- %
% windowed_IQR: (M-K)x3 array, where first column stores window starts,
...
for i = 1:size(windows, 1)
   ...
   % Parse data to consider only the window specified
    ...
    if numel(data_window) >= numData_rqrd
        ...
        windowed_IQR = [windowed_IQR; start_time, end_time, IQR];
    else
